...
print("Calculando evolución temporal de H y H_modificada...")

x_test = CI[3]
p_0, q_0 = np.log(x_test[0]), np.log(x_test[1])
y_test = np.array([p_0, q_0])

pasos_energia = 1500
tiempo = np.arange(pasos_energia) * h

H_evol = np.zeros(pasos_energia)
Hm_evol = np.zeros(pasos_energia)


H_inicial = H_exacta(p_0, q_0)
Hm_inicial = H_m(p_0, q_0, h)

for i in range(pasos_energia):
    H_evol[i] = H_exacta(y_test[0], y_test[1])
    Hm_evol[i] = H_m(y_test[0], y_test[1], h)

    y_test = symplectic_euler_lv(y_test, h)

fig3, ax3 = plt.subplots(figsize=(10, 5))

ax3.plot(tiempo, H_evol - H_inicial, label='$\Delta H$ (Exacto)', color='blue', linewidth=1.5)

ax3.plot(tiempo, Hm_evol - Hm_inicial, label='$\Delta H_m$ (Modificado)', color='black', linewidth=2)

ax3.set_title("Conservación del Hamiltoniano a lo largo del tiempo", fontsize=14)
ax3.set_xlabel("Tiempo $t$", fontsize=12)
ax3.set_ylabel("Variación de la Energía", fontsize=12)
ax3.axhline(0, color='gray', linestyle='--', linewidth=1)  # Línea base del 0
